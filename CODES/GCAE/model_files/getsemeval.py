    ...
    with open(file_path, 'rb') as fopen:
        ...
        for sentence in root:
            ...
            for c in terms:
                ...
                # for RAN
                left_word_offset = len(tk(example['sentence'][:left_index]))
                right_word_offset = len(tk(example['sentence'][right_index:]))
...
def print_unrolled_stats_atsa(unrolled_data):
    ...
    for e in unrolled_data:
        ...
        length_list.append(len(tk((e['sentence']))))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rest_lap', type=str, default='r', help='restaurants or laptops')
