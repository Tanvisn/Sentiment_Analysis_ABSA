...
def load_w2v_embedding(word_list, uniform_scale, dimension_size):
    embed_file = '/home/tanvi/Downloads/GoogleNews-vectors-negative300.bin'
    # model = gensim.models.KeyedVectors.load_word2vec_format(embed_file, binary=True)
    model = gensim.models.KeyedVectors.load_word2vec_format('/home/tanvi/Downloads/GoogleNews-vectors-negative300.bin.gz', binary=True)
    word_vectors = []
...
def load_glove_embedding(word_list, uniform_scale, dimension_size):
    ...
    word_vectors = []
    # words = pd.read_table('/home/tanvi/Downloads/archive1/glove.840B.300d.txt', sep=" ", index_col=0, header=None, quoting=csv.QUOTE_NONE)
    
    for word in word_list:
        if word in word2offset:
            line = linecache.getline('/home/tanvi/Downloads/archive1/glove.840B.300d.txt', word2offset[word]+1)
            # line = words.loc[word].as_matrix()
    ...
    # word_vectors = codecs.open('./glove/vectors' + ".txt", 'r', encoding='utf-8')
    return word_vectors
